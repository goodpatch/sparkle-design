...
| 2.5.8 | ターゲットサイズ（最小） | AA | 操作対象は24x24px以上 | Fail | sm サイズ: `h-4 w-7`（16px x 28px、`index.tsx:25`）で高さが 24px 未満。md サイズ: `h-6 w-11`（24px x 44px、`index.tsx:26`）で 24x24px を満たす。lg サイズ: `h-8 w-[60px]`（32px x 60px、`index.tsx:27`）で 24x24px を満たす。 | **sm サイズの高さが 16px で 24px 未満。** WCAG 2.5.8（AA）の最小ターゲットサイズ要件を満たさない。sm サイズの高さを 24px 以上に変更するか、周囲に十分なスペースを確保して実効タッチ領域を 24px 以上にすることを推奨。 |
...
- **影響**: モバイル環境やタッチ操作でのミスタップリスクが高まる
- **修正提案**:
  1. sm サイズの最小高さを 24px（`h-6`）に変更する
  2. または、CSS の `min-height` / パディングで実効タッチ領域を 24px 以上に拡大する（視覚的なサイズは維持しつつ透明なタッチ領域を拡張）
  3. デザインチームと相談の上、sm サイズの仕様自体を見直す

```tsx
// 修正案（概念）: 透明なタッチ領域を拡張する方法
// sm の場合、視覚は h-4 のまま、タッチ領域を ::before 疑似要素で拡張
// または min-h-6 に変更する
```
