...
| 2.5.8 | ターゲットサイズ（最小） | AA | 操作対象は24x24px以上 | Fail | Thumb サイズは `h-4 w-4`（16x16px）で、WCAG 2.5.8 の最小基準 24x24px を満たさない（`index.tsx:46`）。ただし Track 全体（`h-1` = 4px + `py-1.5` = 12px のパディング領域）もクリック可能であり、Track の高さ方向のタッチ領域は実質的に拡張されている。 | **Thumb の視覚サイズは 16x16px だが、Radix UI の実装では Thumb のヒットエリアは CSS の視覚サイズとは別にポインタイベントで制御される場合がある。** 実機での操作テストで十分なタッチ領域があるか確認が必要。改善案: Thumb に `::before` 疑似要素で 24x24px 以上のヒット領域を追加、または `min-h-6 min-w-6` に変更。 |
...
#### 2.5.8 ターゲットサイズ（最小）(Level AA)

- **問題**: Thumb の視覚サイズが `h-4 w-4`（16x16px）で、WCAG 2.5.8 の最小基準 24x24px を満たさない
- **箇所**: `src/components/ui/slider/index.tsx:46`
- **影響**: モバイルユーザーやモーター障害のあるユーザーが Thumb を正確にタップ/クリックしにくい
- **緩和要因**:
  - Track 全体がクリック可能で、Thumb 以外の場所でも値設定が可能
  - Root 要素に `py-1.5`（12px）のパディングがあり、タッチ領域が実質的に拡張されている
  - Radix UI の内部実装でポインタイベントの領域が CSS サイズより広い可能性がある
- **修正提案**:
  1. Thumb に `::before` 疑似要素で 24x24px 以上のヒット領域を追加
  2. または Thumb サイズを `min-h-6 min-w-6`（24x24px）に変更
  3. 実機テストで現在のヒット領域が十分か確認してから判断
