...
| 3.2.4 | 一貫した識別性 | AA | 同じコンポーネントは一貫したラベル | Pass | `input-password/index.tsx:51-53` -- `triggerAriaLabel` は状態に応じて「パスワードを隠す」/「パスワードを表示する」で一貫。コンポーネントの props インターフェースが統一されている。 | 一貫したラベリング。 |
