...
|---:|---|:---:|---|---|---|---|
| 1.1.1 | 非テキストコンテンツ | A | アイコンのみ等に代替テキストがある | Pass (修正済み) | `index.tsx` — `isNumberVisible=false` かつ `aria-label` / `aria-labelledby` 未指定時に dev warn を出力するよう修正済み。`role="status"` のデフォルト付与により支援技術に意味が伝わるようになった | 修正コミット: `♿ fix(badge): アクセシビリティ改善` |
...
| 4.1.2 | 名前・役割・値 | A | 操作要素がname/role/状態を持つ | Pass (修正済み) | `index.tsx` — `role="status"` をデフォルトで付与するよう修正済み。利用者が `role` prop でオーバーライド可能。`role="status"` により暗黙的に `aria-live="polite"` が適用され、支援技術に通知バッジとしての意味が伝わる | 修正コミット: `♿ fix(badge): アクセシビリティ改善` |
| 4.1.3 | 状態メッセージ | AA | 状態変化が支援技術に伝わる | Pass (修正済み) | `index.tsx` — `role="status"` のデフォルト付与により、暗黙的に `aria-live="polite"` が適用される。Badge の数値が動的に変化した場合、支援技術にその変化が自動的に通知される | 修正コミット: `♿ fix(badge): アクセシビリティ改善` |

---

## 対応内容

以下の修正を実施済みです（コミット: `♿ fix(badge): アクセシビリティ改善`）。

### 修正済み Fail 項目

#### 1) 1.1.1 非テキストコンテンツ — `xs`/`sm` サイズの代替テキスト欠如 → **修正済み**

- **問題**: `xs`/`sm` サイズおよび `isNumberVisible=false` の場合、支援技術に対する代替テキストが提供されない
- **対応**: `isNumberVisible=false` かつ `aria-label` / `aria-labelledby` 未指定時に dev warn（`console.warn`）を出力するよう実装

#### 2) 4.1.2 名前・役割・値 — デフォルト role の欠如 → **修正済み**

- **問題**: Badge に `role` がデフォルトで設定されていない
- **対応**: `role="status"` をデフォルトで付与。利用者が props でオーバーライド可能

#### 3) 4.1.3 状態メッセージ — 動的変更の通知欠如 → **修正済み**

- **問題**: Badge の数値が動的に変化しても支援技術に通知されない
- **対応**: `role="status"` の付与により暗黙的に `aria-live="polite"` が適用され、動的変更が支援技術に通知される
...
1. ~~**高**: `4.1.2` — `role="status"` のデフォルト付与~~ → **修正済み**
2. ~~**高**: `4.1.3` — `role="status"` 付与により暗黙的に解消~~ → **修正済み**
3. ~~**中**: `1.1.1` — `xs`/`sm` サイズの代替テキスト案内~~ → **修正済み**（dev warn 追加）
