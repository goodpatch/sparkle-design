...
または `./scripts/setup.sh <コンポーネント名>` を実行してください。詳細は `.github/instructions/new-component.instructions.md` を参照してください。
...
- React コンポーネントのコメント記法は `.github/instructions/comment-style.instructions.md` を参照してください。
- コミットメッセージの形式は `.github/copilot-commit-message-instructions.md` のルールに従います。
- 変更履歴は `CHANGELOG.md` を参照してください。
- 開発・テスト・AI関連のガイドラインは `.github/instructions/` ディレクトリを参照してください。
