...
export const MotionPreview: Story = {
  parameters: {
    docs: {
      description: {
        story:
          "ボタンで `data-state` を open/closed に切り替えて、オーバーレイのフェードを確認する。Open 150ms ease-out / Close 200ms ease-in。Modal の `ModalOverlay` と同じ timing に揃える。\n\nen: Toggle `data-state` between open/closed to verify overlay fade. Open 150ms ease-out / Close 200ms ease-in. Matches `ModalOverlay` timing.",
      },
    },
  },
  render: args => {
